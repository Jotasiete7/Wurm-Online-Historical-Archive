export const translations = {
  en: {
    title: "Wurm Historical Archive",
    subtitle: "A community-driven immutable vault for the preservation of Wurm Online's history.",
    upload_title: "Fragment Ingestion",
    upload_desc: "Historical logs are preserved immutably. No account required.",
    upload_credit_placeholder: "How should we credit you? (Optional)",
    upload_processing: "Restoring {count} fragments...",
    upload_success: "Archival preservation complete.",
    historical_coverage: "Historical Coverage",
    browse_archive: "Browse Archive",
    archive_desc: "Access the derived institutional reconstructions generated from recovered fragments.",
    no_data: "No fragments recovered for this corpus yet.",
    missing_call: "Still missing. Do you have logs?",
    restore_corpus: "Restore Corpus",
    downloading: "Accessing Vault...",
    merging: "Merging History...",
    deduplicating: "Deduplicating...",
    restored: "Restored!",
    recent_discoveries: "Recent Discoveries",
    discovery_anonymous: "Anonymous",
    discovery_recovered: "Recovered",
    discovery_from: "from",
    discovery_days: "days of history",
    contributors_title: "Hall of Contributors",
    mission_title: "Preserving the <span class=\"accent\">immutable memory</span> of Wurm Online.",
    mission_sub: "This archive is a community-driven effort to salvage and protect historical records — from trade transactions to world events — currently fragmented across player hard drives. Before time erases the traces of our journey, we consolidate them into an eternal vault for future generations.",
    faq_q1: "Why preserve logs?",
    faq_a1: "Wurm Online's history is written in its player logs. Trade, events, and community interactions are fragmented across thousands of local files. This project ensures this history isn't lost when players move on or hard drives fail.",
    faq_q2: "Is this safe?",
    faq_a2: "We only collect trade and event logs. We do not track players, analyze market intelligence, or build profiles. The archive is a library, not a surveillance tool.",
    faq_q3: "What is NOT collected?",
    faq_a3: "Private messages, sensitive coordination, or behavioral telemetry. We focus on the public and semi-public history of the world.",
    faq_q4: "Contribution Credit",
    faq_a4: "All contributions are voluntary. You can choose to be credited in the archive or remain anonymous. The logs you upload are already generated locally by your game client.",
    footer_copy: "&copy; 2024 Wurm Online Historical Archive Project. An independent community initiative."
  },
  pt: {
    title: "Arquivo Histórico de Wurm",
    subtitle: "Um cofre imutável mantido pela comunidade para a preservação da história do Wurm Online.",
    upload_title: "Ingestão de Fragmentos",
    upload_desc: "Logs históricos são preservados de forma imutável. Não é necessária conta.",
    upload_credit_placeholder: "Como devemos creditar você? (Opcional)",
    upload_processing: "Restaurando {count} fragmentos...",
    upload_success: "Preservação arquivística concluída.",
    historical_coverage: "Cobertura Histórica",
    browse_archive: "Explorar Arquivo",
    archive_desc: "Acesse as reconstruções institucionais derivadas geradas a partir de fragmentos recuperados.",
    no_data: "Nenhum fragmento recuperado para este corpus ainda.",
    missing_call: "Ainda faltando. Você tem logs?",
    restore_corpus: "Restaurar Corpus",
    downloading: "Acessando Cofre...",
    merging: "Mesclando História...",
    deduplicating: "Deduplicando...",
    restored: "Restaurado!",
    recent_discoveries: "Descobertas Recentes",
    discovery_anonymous: "Anônimo",
    discovery_recovered: "Recuperou",
    discovery_from: "de",
    discovery_days: "dias de história",
    contributors_title: "Mural dos Contribuidores",
    mission_title: "Preservando a <span class=\"accent\">memória imutável</span> de Wurm Online.",
    mission_sub: "Este arquivo é um esforço comunitário para resgatar e proteger os registros históricos — de trocas comerciais a grandes eventos — que residem fragmentados nos computadores dos jogadores. Antes que o tempo apague os vestígios da nossa jornada, nós os transformamos em um cofre eterno para as gerações futuras.",
    faq_q1: "Por que preservar logs?",
    faq_a1: "A história do Wurm Online está escrita nos logs dos jogadores. Trocas, eventos e interações comunitárias estão fragmentadas em milhares de arquivos locais. Este projeto garante que essa história não se perca quando os jogadores saem ou os discos rígidos falham.",
    faq_q2: "Isso é seguro?",
    faq_a2: "Nós apenas coletamos logs de trade e eventos. Não rastreamos jogadores, não analisamos inteligência de mercado ou criamos perfis. O arquivo é uma biblioteca, não uma ferramenta de vigilância.",
    faq_q3: "O que NÃO é coletado?",
    faq_a3: "Mensagens privadas, coordenação sensível ou telemetria comportamental. Focamos na história pública e semi-pública do mundo.",
    faq_q4: "Crédito de Contribuição",
    faq_a4: "Todas as contribuições são voluntárias. Você pode escolher ser creditado no arquivo ou permanecer anônimo. Os logs que você envia já são gerados localmente pelo seu cliente de jogo.",
    footer_copy: "&copy; 2024 Projeto de Arquivo Histórico Wurm Online. Uma iniciativa comunitária independente."
  }
}
